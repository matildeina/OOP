...
class ShowImage(QMainWindow):
    def __init__(self):
        ...
        self.actionCrom_Image.triggered.connect(self.cropImage)
    ...
    #fungsi yg dipanggil saat tombol ditekan
    @pyqtSlot()
    ...
    def displayImage(self, windows=1):
        #memilih format qimage tergantung pada jlh channel gambar
    ...
    #konversikan ke grayscale dan tampilkan histogram
    ...
    #konversi ke grayscale dan ubah ke biner berdasarkan T
    def biner(self):
    ...
    #Konversi gambar menjadi negatif dengan 255 - pixel.
    def negative(self):
        img = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY)
        h, w = img.shape[:2]
        for i in np.arange(h):
    ...
    #Menampilkan histogram untuk masing-masing channel warna (R, G, B).
    ...
    def aritmatika(self):
        image1 = cv2.imread ('paru-paru.png', 1)
        image2 = cv2.imread('paru-paru - Copy.png', 1)
    ...
    def cropImage(self):
        ...
        # Tentukan ukuran area crop (ambil 50% dari tengah)
        crop_size = 0.5  # Misalnya, crop ke 50% dari ukuran asli
        new_h, new_w = int(h * crop_size), int(w * crop_size)

        # Hitung koordinat untuk crop tengah
        start_row = (h - new_h) // 2
        start_col = (w - new_w) // 2
        ...
        # Crop gambar
        self.image = self.image[start_row:end_row, start_col:end_col].copy()
